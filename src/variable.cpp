...
{
    if (mode == 0) cout<<value<<endl;
    else if (mode > 0 and value < 0) cout<<"("<<value<<")";
    else cout<<value;
}
...
{
    ostringstream sout; 
    if (mode == 0) sout<<value<<endl;
    else if (value < 0) sout<<"("<<value<<")";
    else sout<<value;
...
Expr Double::multiplication_own(const Expr& t_abstract) const
{
    ...
    switch(t_abstract->getType())
    {
        ...
        default:
        cout<<"Warning: numerical type \""<<t_abstract->getType()<<"\" not recognized in number oeprations.\n";
        return ZERO;
    }
}
...
double Integer::evaluateScalar() const { return value;}

Expr Integer::evaluate() { return shared_from_this();}
...
{
    ostringstream sout; 
    if (mode == 0 and valued) sout<<name<<" = "<<value<<endl;
    else if (mode == 0) sout<<name<<endl;
    else sout<<name;
...
{
    if (valued) return auto_number_(value);
    else return ZERO;
}

int Variable::getParity(const Expr& t_variable) const
{
    if (*this == t_variable) return -1;
    else return 1;
}

void Variable::operator=(int t_value) {
    valued = true;
    value = t_value;
}
...
{
   if (not valued) cout<<"Warning variable \""<<name<<"\" not valued, evaluated to 0.\n";
   return value;
}
...
void Constant::operator=(int t_value) {
    valued = true;
    value = t_value;
}
...
{
    if (mode == 0) cout<<"CFactorial: "<<value<<"!"<<endl;
    else cout<<value<<"!";
}
...
{
    if (value < 0) return times_(pow_(int_(-1),int_(abs(value))),cfactorial_(-value));
    if (value == 0 or value == 1) return int_(1);
    if (value == 2) return int_(2);
...
{
    if (mode == 0) cout<<"Fraction: "<<num<<"/"<<denom<<endl;
    else if (mode > 3) cout<<"("<<num<<"/"<<denom<<")";
    else cout<<num<<"/"<<denom;
}
...
{
    ostringstream sout; 
    if (mode == 0) sout<<"\\frac{"<<num<<"}{"<<denom<<"}";
    else if (mode > 3) sout<<"\\left("<<"\\frac{"<<num<<"}{"<<denom<<"}\\right)";
    else sout<<"\\frac{"<<num<<"}{"<<denom<<"}";

    return sout.str();
}

double CFraction::evaluateScalar() const { return (num*1.)/denom;}

Expr CFraction::evaluate() { return auto_number_(num*1./denom);}
...
Expr _cfraction_(int num, int denom)
{
    ...
    if (foo->getDenom() == 0) return INF;
    if (foo->getNum() == 0) return ZERO;
    if (foo->getDenom() == 1) return auto_number_(foo->getNum());
    return foo;
}
...
{
    cout<<"Warning: evaluating scalar (real) of complex expression! replacing i by 0.\n";
    return 0;
}
...
{
    if (value == round(value)) return int_(value);
